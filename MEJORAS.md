...
- **2026-06-30 (oleada 15)** — Pestañas en el detalle, selección múltiple de subtareas y dashboards por categoría:
  ...
  - Verificado: `py_compile` OK (backend) y balance de bloques Svelte correcto (`{#if}`=`{/if}`=81, `{#each}`=`{/each}`=10, `{#snippet}`=`{/snippet}`=1). Frontend pendiente de `npm run check` en tu Mac. Warnings a11y esperables por el rubber-band/clic en `div` (no bloquean). Pendiente (a confirmar): borrado múltiple en las listas principales (`/` y `/dashboard`).
