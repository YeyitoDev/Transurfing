...
- **2026-06-30 (oleada 13)** — Feed vivo (experimental) con búsquedas reales en internet (`/feed` pestaña **Vivo**):
  - Nuevo `feed_vivo_service.py`: genera consultas desde tus proyectos (hábitos/emprendimientos/ideas) y trae resultados reales de **arXiv** (ciencia), **Hacker News** (tech/startups/ideas) y **Wikipedia** (contexto). Búsqueda **web opcional por API key** (`TAVILY_API_KEY` o `BRAVE_API_KEY`): si la pones, se activa sola.
  - **Criterios objetivos y medibles**: cada ítem se puntúa 0–100 con `relevancia` (cobertura de términos), `recencia` (decaimiento por vida media según fuente), `popularidad` (puntos/comentarios o score) y `autoridad` (fiabilidad de la fuente). Pesos: 0.30/0.30/0.20/0.20, devueltos al cliente para que el ranking sea auditable.
  - **Pocas consultas**: cache en `data/feed_vivo_cache.json` con TTL `FEED_VIVO_TTL_MIN` (6 h) y anti-spam `FEED_VIVO_MIN_REFRESH_MIN` (20 min) en el botón Actualizar; máx. `FEED_VIVO_MAX_CONSULTAS` (3) por refresco. Desactivable con `FEED_VIVO_ENABLED=0`.
  - **Ver más allá**: panorama IA (2-3 frases) + **5 preguntas provocadoras** generadas a partir de los hallazgos reales (best-effort, 1 llamada LLM, cacheada).
  - Endpoint `GET /api/feed-vivo?force=` + `api.feedVivo()`. UI con pestañas Curado/Vivo, badges por fuente, score con desglose de criterios (mini-barras), fecha, métricas y enlaces.
  - Verificado: `py_compile` OK y funciones puras de scoring/relevancia/fecha probadas (score=52, relevancia=1.0). El parseo en vivo no se pudo probar aquí porque este entorno crashea a nivel nativo en cualquier request HTTPS standalone (httpx y urllib); en la app bajo uvicorn (donde los LLM ya usan httpx) funcionará. Validar con el server arriba: `curl /api/feed-vivo`.
- **2026-06-30 (oleada 14)** — Kanban de subtareas (4 columnas) + agente Scrum/PM en el detalle de tarea (`TaskDetailModal`):
  - **Kanban**: las subtareas dejan de ser una lista y pasan a un tablero de 4 columnas: **Futuras** (`bloqueada`) · **Pendientes** (`pendiente`) · **En progreso** (`en_progreso`) · **Resueltas** (`completada`). El tablero ocupa todo el ancho; Chat y Resumen quedan debajo en 2 columnas.
  - **Drag & drop**: cada tarjeta tiene un asa (icono grip) que se arrastra entre columnas; soltar cambia el `estado` (y `completada` para Resueltas) con actualización optimista y recálculo de progreso. La tarjeta conserva toda su funcionalidad (ejecutar agente, commit, iterar, historial, probar código, editar). Implementado con un `{#snippet subtareaCard(sub)}` reutilizado por columna.
  - **Agente Scrum + Project Manager**: nuevo `scrum_agent_service.py` (`analizar_quick_wins`) que, según el **objetivo** y el backlog, devuelve diagnóstico del sprint, **quick wins priorizados** (con `impacto` y `esfuerzo` alto/medio/bajo y, si aplica, el id de una subtarea existente), recomendaciones de PM y riesgos. Valida niveles e ids contra el backlog real.
  - Endpoint `POST /api/tareas/{id}/scrum` + `api.scrumQuickWins()`. Panel en el modal con botón **Analizar quick wins**, tarjetas de quick win con badges de impacto/esfuerzo y botón **Crear subtarea** para los que aún no existen.
  - Verificado: `py_compile` OK (backend) y balance de bloques Svelte correcto (`{#if}`/`{/if}`, `{#each}`/`{/each}`, `{#snippet}`/`{/snippet}`). Frontend pendiente de `npm run check` en tu Mac (aquí no hay `node_modules`). Posibles warnings a11y por el drag&drop en `div`/`span` (no bloquean el build).
- **2026-06-30 (oleada 15)** — Pestañas en el detalle, selección múltiple de subtareas y dashboards por categoría:
  - **Pestañas del modal** (`TaskDetailModal`): el cuerpo (antes muy largo) se divide en pestañas con barra **sticky**: `Tablero` · `Chat` · `Resumen` · `Scrum/PM` · `GitHub`. El header (título, badges, progreso, banners) queda fijo arriba; Chat y Resumen ahora a 1 clic y a ancho completo (adiós scroll largo).
  - **Selección múltiple de subtareas**: en el tablero, **rubber-band** (arrastrar sobre el fondo dibuja un recuadro que selecciona las tarjetas que toca) + **Ctrl/⌘+clic** para marcar/desmarcar. Barra de acciones con **N seleccionadas**, **Mover a** (las 4 columnas) y **Eliminar** (en lote, con confirmación y update optimista). Tarjetas marcadas con anillo `accent`. La selección se reinicia al cambiar de tarea (`$effect` sobre `tarea.id`).
  - **Dashboards por categoría**: `/dashboard` gana sub-pestañas (Todos · Emprendimientos · Ideas · Hábitos · Investigación · Tareas) con conteo por pestaña. Al cambiar, **todo** se recalcula para esa categoría (headline, KPIs, insights, tarjetas) porque `proyectos` se filtra y el resto deriva de él. El análisis IA también se hace por categoría: endpoint `GET /api/agente/resumen-dashboard?etiqueta=` + `api.resumenDashboard(etiqueta)`. Se quitó el select "Tipo" (redundante con las pestañas).
  - **Borrado múltiple de tareas (listas principales)**: nueva acción reutilizable `frontend/src/lib/actions/rubberband.ts` (selección por arrastre con recuadro `position:fixed`, umbral de 6px para no romper el clic, y supresión del clic posterior para no abrir el detalle tras arrastrar). Conectada en `/` (Pendientes) y `/dashboard`: arrastrar sobre las tarjetas selecciona varias (anillo `accent`) y aparece una barra fija con **N seleccionada(s)** + **Eliminar** (en lote, con confirmación) + **Cancelar**. Borra vía `api.eliminarTarea` + `onTaskChange(null, id)`.
  - Verificado: `py_compile` OK (backend) y balance de bloques Svelte correcto (modal `{#if}`=`{/if}`=81, `{#each}`=`{/each}`=10, `{#snippet}`=`{/snippet}`=1; `/` 3/3 y 1/1; dashboard 12/12 y 3/3). Frontend pendiente de `npm run check` en tu Mac. Warnings a11y esperables por el rubber-band/clic en `div` (no bloquean). El recuadro usa `rgb(var(--c-accent))` (variables del tema).
